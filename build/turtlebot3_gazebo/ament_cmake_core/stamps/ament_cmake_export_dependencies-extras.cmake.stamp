...
set(_exported_dependencies "gazebo_ros_pkgs;geometry_msgs;nav_msgs;rclcpp;sensor_msgs;tf2;std_msgs;std_srvs")
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(turtlebot3_gazebo_INCLUDE_DIRS "${_include_dirs}")
          endif()
